...
def FetchHtml(url):
    ...
    try:
        ...
        FileName = "./soccer/" + soup.title.contents[0].split("|")[0][0:-1].replace(" ","_").replace("/", "_").replace(":", "_").replace("?", "").replace("\"", "").replace("\'", "") + '.json'
...
with open('url_socc_clean.txt', 'r') as csv_file:
